...
from modeling_distilbert_align import DistilBertForSequenceClassification
...
def train(args, train_dataset, model, tokenizer):
    """ Train the model """
    tb_writer = SummaryWriter("./runs3/distilbert_align/")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False):
    ...
    cached_features_file = os.path.join(args.data_dir, 'cached_{}_distilbert_{}_{}_align'.format(
...
def main():
    ...
    parser.add_argument("--output_dir", default="distilbert_align2", type=str,
    ...
    config_class, model_class, tokenizer_class = DistilBertConfig, DistilBertForSequenceClassification, DistilBertTokenizer
    config = config_class.from_pretrained("./pretrained/distilbert/config.json",num_labels=num_labels,finetuning_task=args.task_name)
    tokenizer = tokenizer_class.from_pretrained("./pretrained/distilbert/")
    model = model_class.from_pretrained("./pretrained/distilbert/distilbert-base-uncased-pytorch_model.bin", from_tf=False,
                                        config=config)
