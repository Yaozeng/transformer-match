...
from modeling_bert_add import BertForSequenceClassification
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    if args.evaluate_during_training:
                        ...
                        for key, value in results.items():
                             with open(os.path.join(args.output_dir, "{}.txt".format(key)), 'a+') as w:
                                 w.write("%d\t%f\n"%(global_step, value))
...
def predict(args, model, tokenizer):
    ...
    eval_outputs_dirs = (args.output_dir, args.output_dir + '-MM') if args.task_name == "mnli" else (args.output_dir,)
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        eval_dataset = load_and_cache_examples(args, eval_task, tokenizer, is_train_or_dev=False,evaluate=False)
        ...
        if args.output_mode == "classification":
            ...
            preds = np.squeeze(preds)
        output_eval_file = os.path.join(eval_output_dir, "{}.tsv".format(args.task_name))
        df=pd.read_csv("{}/test.tsv".format(args.data_dir),delimiter='\t',usecols=['id'])
        out_df=pd.DataFrame({"qid":df["id"].values})
        out_df['prediction'] = preds
        out_df.to_tsv(output_eval_file, index=False,sep='\t')

def load_and_cache_examples(args, task, tokenizer,is_train_or_dev=True, evaluate=False):
    ...
    if is_train_or_dev:
        ...
        dataset = TensorDataset(all_input_ids, all_attention_mask, all_align_mask, all_token_type_ids)
    return dataset
...
def main():
    ...
    # Training
    if args.do_train:
        train_dataset = load_and_cache_examples(args, args.task_name, tokenizer, is_train_or_dev=True,evaluate=False)
