...
from modeling_bert_fusion import BertForSequenceClassification
                    ...
                    if args.evaluate_during_training:
                        ...
                        for key, value in results.items():
                            if key=="mnli-acc":
                                ...
                                with open(os.path.join(args.output_dir, "mnli-acc.txt"), 'a+') as w:
                            ...
                            if key == "mnli-mm-acc":
                                max_mm_acc=max([max_mm_acc,value])
                                with open(os.path.join(args.output_dir, "mnli-mm-acc.txt"), 'a+') as w:
                                    w.write("%d\t%f\t%f\n" % (global_step, value, max_mm_acc))
                    with open(os.path.join(args.output_dir, "loss.txt"), 'a+') as w:
                        w.write("%d\t%f\n"%(global_step, (tr_loss - logging_loss) / logging_steps))
                    with open(os.path.join(args.output_dir, "lr.txt"), 'a+') as w:
...
def main():
    ...
    parser.add_argument("--output_dir", default="output_mnli_fusion", type=str,
