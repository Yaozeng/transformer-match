...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.linear_r = nn.Linear(self.dim//self.n_heads*2, self.dim//self.n_heads)
        self.linear_g = nn.Linear(self.dim//self.n_heads*2, self.dim//self.n_heads)
    ...
    def forward(self, query, key, value, mask,align_mask, head_mask=None):
        """
        Parameters
        ----------
        ...
        extend_align_mask = extend_align_mask.to(dtype=next(self.parameters()).dtype)  # fp16 compatibility
        extend_align_mask = (1.0 - extend_align_mask) * -10000.0
        scores2=scores+extend_align_mask
        ...
        weights2 = nn.Softmax(dim=-1)(scores2)  # (bs, n_heads, q_length, k_length)
        weights2 = self.dropout(weights2)  # (bs, n_heads, q_length, k_length)
        ...
        context2=torch.matmul(weights2, v)

        r_f = torch.cat([context, context2], dim=-1)
        r = torch.tanh(self.linear_r(r_f))
        g = torch.sigmoid(self.linear_g(r_f))
        context = g * r + (1 - g) * context
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, attn_mask=None,align_mask=None, head_mask=None):
        """
        Parameters
        ----------
        ...
        """
        # Self-Attention
        sa_output = self.attention(query=x, key=x, value=x, mask=attn_mask, align_mask=align_mask,head_mask=head_mask)
        if self.output_attentions:
...
class DistilBertModel(DistilBertPreTrainedModel):
    ...
    def forward(self,
                input_ids, attention_mask=None, align_mask=None,head_mask=None):
...
class DistilBertForSequenceClassification(DistilBertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, align_mask=None,head_mask=None, labels=None):
